import json
from scientific_review.llm.client import LLMClient
from scientific_review.utils.parser import extract_json
from scientific_review.agents.state import ReviewState
from scientific_review.config.settings import MODELS


class CriticAgent:
    def __init__(self):
        self.client = LLMClient(
            model=MODELS["review"]["critic"],
            temperature=0.3,
            max_tokens=1400
        )
        with open("scientific_review/prompts/agents/critic.txt", encoding="utf-8") as f:
            self.prompt_template = f.read()

    async def run(self, state: ReviewState) -> ReviewState:
        if not state.review_draft:
            raise ValueError("Draft review must exist before CriticAgent")

        scores_str = json.dumps(state.scores, ensure_ascii=False, indent=2)

        prompt = (
            self.prompt_template
            .replace("{{DRAFT_REVIEW}}", state.review_draft)
            .replace("{{SCORES_JSON}}", scores_str)
            .replace("{{TEXT}}", state.text[:12000])
        )

        response = await self.client.generate(prompt)
        parsed = extract_json(response["text"])

        if not isinstance(parsed, dict):
            parsed = {"major_issues": [], "improvement_suggestions": []}

        state.review_critic = json.dumps(parsed, ensure_ascii=False, indent=2)

        state.agent_outputs.append({
            "agent": "critic",
            "raw": response["text"],
            "parsed": parsed,
            "latency": response.get("latency", 0.0),
        })

        return state
